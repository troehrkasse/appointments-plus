...
class Packages
{
    ...
    {
        /** Load Classes */
        foreach (glob(AWESOME_SLIDES_DIR . "/classes/*.php") as $file):
            require_once($file);
        endforeach;
	}
	...
	*/
	public function appointment_packages_add_to_cart_function( $message, $products ) {
	...
	* 
	* If a package is purchased, look for qualifying appointments in the order and account for them before saving the new package.
	* 
	* If appointments are purchased and option to pay with package - COD - is selected, look for packages in user account and use them.
	*/
	public function add_package_to_user($order_id) {
		...
		*/
		if (sizeof($packages) > 0) {
			foreach ($packages as $package=>$appointment) {
				...
				// Store the new package and remaining quantity in current packages
		...
		*/
		...
		// If no more appointments  or packages are in the order, mark the appointment as paid
		if (sizeof($appointments) == 0 && sizeof($packages) == 0) {
    ...
    protected function init()
    {
		...
		/* Customize the add-to-cart button for appointment packages */
		add_filter( 'wc_add_to_cart_message_html', array(&$this, 'appointment_packages_add_to_cart_function'), 10, 2 ); 
